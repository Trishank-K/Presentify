...
const ThemePicker = ({ selectedTheme, themes, onThemeSelect }: Props) => {
  ...
  const handleGenerateLayouts = async () => {
    ...
    try {
      ...
      if (res.status != 200 || "error" in res) {
    ...
    >
      <div className="p-8 space-y-6 flex-shrink-0">
        <div className="space-y-2">
          ...
          >
            Choose a theme that fits your presentation style. You can customize
            it later.
          </p>
        </div>
        <Button
          className="w-full h-12 text-lg font-medium shadow-lg hove:shadow-xl transition-all duration-300"
          style={{
